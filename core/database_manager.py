...
class DatabaseManager:
    def __init__(self, config):
        ...
        self.config = config
        self.created_databases = []  # Важно: добавьте этот атрибут!
    ...
    def create_databases(self, databases_list):
        ...
        # Нужно импортировать глобальные конфиги
        from config import DATABASES_CONFIG

        for db_name in databases_list:
            if db_name in DATABASES_CONFIG:
                if self.create_database(db_name, DATABASES_CONFIG[db_name]):
                    success_count += 1
            else:
                print(f"❌ База данных '{db_name}' не найдена в конфигурации")

        self._show_summary(success_count, databases_list)
        return success_count

    def _show_summary(self, success_count, databases_list):
    ...
    # ВСЕ print() оставляем на месте! Они будут перехвачены.
    def create_database(self, db_name):
        print(f"[INFO] Создание базы данных: {db_name}")  # <-- Оставляем!
        # ... ваша существующая логика
        print(f"[SUCCESS] База '{db_name}' создана успешно!")

    # Остальные методы (create_databases, clean_database и т.д.)
    # остаются без изменений, кроме удаления вызовов загрузки конфига.
